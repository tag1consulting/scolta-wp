...
class AdminRebuildNoticeTest extends TestCase {
	...
	// -------------------------------------------------------------------------
	// PR: fix/dismiss-notice-id-mismatch
	// -------------------------------------------------------------------------

	/**
	 * notice_id generated by handle_rebuild_now() must survive sanitize_key() unchanged.
	 *
	 * Pre-fix: uniqid('scolta_rebuild_', true) → period in ID → sanitize_key strips it → FAIL.
	 * Post-fix: 'scolta_rebuild_' . bin2hex(random_bytes(8)) → hex only → PASS.
	 *
	 * This test evaluates the actual production expression extracted from source,
	 * so it fails if the source reverts to the bad pattern.
	 */
	public function test_notice_id_survives_sanitize_key(): void {
		$source = file_get_contents( dirname( __DIR__ ) . '/admin/class-scolta-admin.php' );

		// Find the $notice_id assignment inside handle_rebuild_now().
		$this->assertMatchesRegularExpression(
			'/function handle_rebuild_now\(\)/s',
			$source,
			'handle_rebuild_now() must exist in class-scolta-admin.php'
		);

		$found = preg_match(
			'/function handle_rebuild_now\(\).*?\$notice_id\s*=\s*([^;]+);/s',
			$source,
			$matches
		);
		$this->assertSame( 1, $found, 'Must find $notice_id assignment in handle_rebuild_now()' );

		$expression = trim( $matches[1] );

		// Evaluate the production expression several times and verify sanitize_key is a no-op.
		for ( $i = 0; $i < 10; $i++ ) {
			// phpcs:ignore Squiz.PHP.Eval.Discouraged -- intentional source-reflection in tests only
			$notice_id = eval( "return {$expression};" );
			$this->assertSame(
				$notice_id,
				sanitize_key( $notice_id ),
				"Production notice_id must survive sanitize_key unchanged: '{$notice_id}' (expr: '{$expression}')"
			);
		}
	}

	/**
	 * Dismiss roundtrip: setting user meta with the production notice_id must suppress the notice.
	 *
	 * Simulates what handle_dismiss_rebuild_notice() does (sanitize_key + update_user_meta)
	 * using the actual notice_id format produced by handle_rebuild_now().
	 *
	 * Pre-fix: sanitize_key strips the period from the ID → stored value ≠ transient ID → notice still shows → FAIL.
	 * Post-fix: sanitize_key is a no-op on hex IDs → stored value = transient ID → notice hidden → PASS.
	 */
	public function test_dismiss_roundtrip(): void {
		$source = file_get_contents( dirname( __DIR__ ) . '/admin/class-scolta-admin.php' );
		preg_match(
			'/function handle_rebuild_now\(\).*?\$notice_id\s*=\s*([^;]+);/s',
			$source,
		...
		// Simulate handle_dismiss_rebuild_notice(): sanitizes the ID and stores it in user meta.
		$sanitized = sanitize_key( $notice_id );
		update_user_meta( get_current_user_id(), 'scolta_dismissed_rebuild_notice', $sanitized );

		ob_start();
		Scolta_Admin::maybe_show_rebuild_notice();
		$output = ob_get_clean();

		$this->assertEmpty( $output, 'Notice must not show after dismiss roundtrip (sanitized ID must equal original)' );
	}

	/**
	 * Source-parse: handle_rebuild_now() must NOT generate notice IDs using uniqid() with more entropy.
	 *
	 * uniqid($prefix, true) appends a high-resolution suffix containing a period,
	 * which sanitize_key() strips — breaking the dismiss roundtrip.
	 * This test prevents reintroduction of the pattern.
	 */
	public function test_notice_id_generation_does_not_use_uniqid_more_entropy(): void {
		$source = file_get_contents( dirname( __DIR__ ) . '/admin/class-scolta-admin.php' );

		// Extract handle_rebuild_now() body only.
		preg_match( '/function handle_rebuild_now\(\)(.*?)^\t}/ms', $source, $fn_matches );
		$fn_body = $fn_matches[1] ?? $source;

		$this->assertDoesNotMatchRegularExpression(
			'/uniqid\s*\([^)]*,\s*true\s*\)/',
			$fn_body,
			'handle_rebuild_now() must not use uniqid(..., true) — the period in the suffix breaks sanitize_key()'
		);
	}
}
