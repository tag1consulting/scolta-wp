...
class Scolta_Auto_Rebuild {
    ...
    public static function init(): void {
        ...
        add_action('save_post', [__CLASS__, 'on_content_change'], 20, 2);
        add_action('before_delete_post', [__CLASS__, 'on_content_change'], 20);
        add_action(self::DEBOUNCE_ACTION, [__CLASS__, 'trigger_rebuild']);
    }
    ...
    public static function on_content_change($post_id, $post = null): void {
        ...
        as_unschedule_all_actions(self::DEBOUNCE_ACTION, [], 'scolta');
        $delay = max(60, (int) ($settings['auto_rebuild_delay'] ?? 300));
        as_schedule_single_action(time() + $delay, self::DEBOUNCE_ACTION, [], 'scolta');
    }
