...
class Scolta_Rebuild_Scheduler {
    ...
    public static function init(): void {
        add_action(self::ACTION_START, [__CLASS__, 'handle_start']);
        add_action(self::ACTION_CHUNK, [__CLASS__, 'handle_chunk'], 10, 1);
        add_action(self::ACTION_FINALIZE, [__CLASS__, 'handle_finalize']);
    }
    ...
    public static function start_rebuild(bool $force = false): bool {
        ...
        if (function_exists('as_schedule_single_action')) {
            as_schedule_single_action(time(), self::ACTION_START, [], 'scolta');
        }
        return true;
    }
    ...
    public static function handle_start(): void {
        ...
        $use_php = ($indexer_setting === 'php' ||
                    ($indexer_setting === 'auto' && !(new PagefindBinary(null, ABSPATH))->resolve()));

        if (!$use_php) {
            self::finish(__('Binary indexer not supported in background mode. Use wp scolta build.', 'scolta'));
            return;
        }
        ...
        update_option('scolta_build_status', [
            'status'           => 'running',
            'started_at'       => time(),
        ...
        if (function_exists('as_schedule_single_action')) {
            as_schedule_single_action(time(), self::ACTION_CHUNK, [0], 'scolta');
        }
    }
    ...
    public static function handle_chunk(int $chunk_index): void {
        ...
        if (function_exists('as_schedule_single_action')) {
            if ($chunk_index + 1 < count($chunks)) {
                as_schedule_single_action(time(), self::ACTION_CHUNK, [$chunk_index + 1], 'scolta');
            } else {
                as_schedule_single_action(time(), self::ACTION_FINALIZE, [], 'scolta');
            }
        }
    }
    ...
    public static function handle_finalize(): void {
        ...
        self::finish($result->success ? $result->message : (__('Failed: ', 'scolta') . $result->error));
    }
