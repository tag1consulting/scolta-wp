...
### Changed
- **Synced `assets/js/scolta.js` (and its `.sha256`) and the bundled WASM (`assets/wasm/scolta_core.js`/`scolta_core_bg.wasm`) to the canonical scolta-php source.** Brings the committed copies up to date with the query-word importance *ranking* change ([scolta-php#164](https://github.com/tag1consulting/scolta-php/pull/164) / [scolta-core#31](https://github.com/tag1consulting/scolta-core/pull/31)): the per-query-word `content`/`incidental` classification is now wired into the scorer, so a result matching only an incidental query word (e.g. `grilled` in `grilled vegetables`) ranks below one matching the content word (`vegetables`). Re-ranking only — result counts are unchanged; an absent/empty classification or `incidental_match_weight == 1.0` reproduces prior scores exactly. The new browser-side `incidental_match_weight` setting (default `0.3`) is not yet surfaced as an admin field — a follow-up.
